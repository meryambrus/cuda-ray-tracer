...
struct vec3 {
	...
	__host__ __device__ vec3(float x0 = 0, float y0 = 0, float z0 = 0) { x = x0; y = y0; z = z0; }
	__host__ __device__ vec3(vec2 v) { x = v.x; y = v.y; z = 0; }
...
__host__ __device__ inline vec3 normalize(const vec3& v) { return v * (1 / length(v)); }

__host__ __device__ inline vec3 cross(const vec3& v1, const vec3& v2) {
	return vec3(v1.y * v2.z - v1.z * v2.y, v1.z * v2.x - v1.x * v2.z, v1.x * v2.y - v1.y * v2.x);
}

__host__ __device__ inline vec3 operator*(float a, const vec3& v) { return vec3(v.x * a, v.y * a, v.z * a); }
...
struct vec4 {
	...
	__host__ __device__ vec4(vec3 xyz, float w0 = 0) { x = xyz.x, y = xyz.y, z = xyz.z, w = w0; }
